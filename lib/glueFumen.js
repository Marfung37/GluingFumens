...
function replaceAllExcept(field, pieces) {
    // replace all pieces with X except given pieces
    // clear lines afterwards
    var fieldCpy = field.copy();
    var re = new RegExp(String.raw(templateObject_1 || (templateObject_1 = __makeTemplateObject(["[^X_", "]"], ["[^X_", "]"])), pieces));
    var fieldHeight = height(field);
    for (var y = 0; y < fieldHeight; y++) {
        for (var x = 0; x < WIDTH; x++) {
            var piece = fieldCpy.at(x, y);
            if (piece.match(re)) {
                fieldCpy.set(x, y, "X");
            }
        }
    }
    var newField = removeLineClears(fieldCpy).field;
    return newField;
}
function countMaxExpectedSoutions(field) {
    ...
    for (var char in frequencyCounter) {
        if (char.match(/[TILJSZO]/) && frequencyCounter[char] > 1) {
            var pieceOnlyField = replaceAllExcept(field, char);
...
function glueFumen(customInput, fast, expectedSolutions, visualize) {
    ...
    for (var _a = 0, inputFumenCodes_1 = inputFumenCodes; _a < inputFumenCodes_1.length; _a++) {
        ...
        for (var _b = 0, inputPages_1 = inputPages; _b < inputPages_1.length; _b++) {
            ...
            var fieldExpectedSolns = countMaxExpectedSoutions(field);
