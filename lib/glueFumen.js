...
exports.encodeAbsOp = encodeAbsOp;
exports.decodeAbsOp = decodeAbsOp;
...
function encodeAbsOp(operation) {
    var ct = (0, defines_1.encodeOp)(operation);
    ct = (ct << 5) + operation.absY;
    return ct;
}
function decodeAbsOp(ct) {
    ct >>= 5; // remove the absolute Y position
    return (0, defines_1.decodeOp)(ct);
}
...
function placePiece(field, minoPositions, mino) {
    if (mino === void 0) { mino = 'X'; }
...
function glue(x0, y0, field, height, piecesArr, allPiecesArr, totalLinesCleared, visualizeArr, expectedSolutions, visualize, order, hold) {
    ...
    for (var y = y0; y < height; y++) {
        for (var x = (y == y0) ? x0 : 0; x < WIDTH; x++) {
            ...
            // is actually a piece
            if (!(0, defines_1.isMinoPiece)(piece))
                continue;
            piece = piece;
            // if specify order and the piece isn't the next possible piece in order
            var pieceIndexUsed = -1;
            if (order !== null) {
                if (!order.slice(0, hold + 1).includes(piece))
                    continue;
            ...
            for (var state = 0; state < rotationStates.length; state++) {
                ...
                var operPiece = {
                    type: piece,
                    rotation: defines_1.Rotation[state],
                    x: centerMino(minoPositions).x,
                    y: centerMino(minoPositions).y,
                    absY: absY,
                };
                newPiecesArr.push(encodeAbsOp(operPiece));
                var newOrder = (order !== null) ? order.slice(0, pieceIndexUsed) + order.slice(pieceIndexUsed + 1) : null;
...
function glueFumen(customInput, expectedSolutions, visualize, order, hold) {
