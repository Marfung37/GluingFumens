...
function glue(x0, y0, field, height, piecesArr, allPiecesArr, cutPiecesArr, allCuts, totalLinesCleared, visualizeArr, expectedSolutions, visualize, order) {
    ...
    for (var y = y0; y < height; y++) {
        for (var x = (y == y0) ? x0 : 0; x < WIDTH; x++) {
            ...
            for (var state = 0; state < rotationStates.length; state++) {
                ...
                for (var _a = 0, absMinoPositions_1 = absMinoPositions; _a < absMinoPositions_1.length; _a++) {
                    var pos = absMinoPositions_1[_a];
                    for (var i = 0; i < totalLinesCleared.length && totalLinesCleared[i] <= pos.y; i++)
                        pos.y++;
                }
                var absY = centerMino(absMinoPositions).y;
                // checking if the piece is cut
                var cutPiece = false;
                for (var i = 0; i < TETROMINO; i++) {
                    var offset = rotationStates[state][i];
                    var pos = absMinoPositions[i];
                    var px = x + offset[0];
                    var py = absMinoPositions[1].y + offset[1]; // index 1 is original y shifted
                ...
                if (thisLinesCleared.length > 0) {
                    ...
                    for (var _b = 0, thisLinesCleared_1 = thisLinesCleared; _b < thisLinesCleared_1.length; _b++) {
                        var lineNum = thisLinesCleared_1[_b];
                        var i = void 0;
                ...
                var newCutPieceArr = __spreadArray([], cutPiecesArr, true);
                newCutPieceArr.push(cutPiece);
                glue(startPos.x, startPos.y, newField, height - thisLinesCleared.length, newPiecesArr, allPiecesArr, newCutPieceArr, allCuts, newTotalLinesCleared, visualizeArr, expectedSolutions, visualize, order ? order === null || order === void 0 ? void 0 : order.slice(1) : null);
...
function glueFumen(customInput, expectedSolutions, visualize, order) {
    ...
    if (visualize) {
        allFumens.push(tetris_fumen_1.encoder.encode(visualizeArr));
        return { allFumens: allFumens, allCuts: allCuts };
    }
    // output glued fumens
    return { allFumens: allFumens, allCuts: allCuts };
}
