...
const field: Field = decoder.decode(
  'v115@BgBtQ4g0QpglB8Q4glQpAtA8g0glBeA8Beg0wwwhgl?g0Q4AtA8QpA8CeglBeglCemli0glBehlDeg0QpBeA8wwwhg?lg0Q4AtBeA8wwwhglg0Q4AtQpAeA8wwwhglg0Q4AtQpAeA8?wwwhglg0Q4AtQpLeAgH'
)[0].field;
const encodedField = new EncodedField(field, HEIGHT);

function allAt(at: (x: number, y: number) => Mino | string) {
  for (let x = 0; x < WIDTH; x++) for (let y = 0; y < HEIGHT; y++) at(x, y);
}

function allSet(set: (x: number, y: number, mino: MinoType) => void) {
  for (let x = 0; x < WIDTH; x++) for (let y = 0; y < HEIGHT; y++) set(x, y, 'X');
}
